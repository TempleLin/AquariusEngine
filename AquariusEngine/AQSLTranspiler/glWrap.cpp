...
#include <AQSLTranspilerJNI_GLWrap.h>
...
JNIEXPORT void JNICALL Java_AQSLTranspilerJNI_GLWrap_defaultOpenGLSettings
(JNIEnv*, jobject) {
    try {
        aqOpenGL->setOpenGL()
            .ver_Profile(3, 3, GLFW_OPENGL_CORE_PROFILE, &_glfwError, _glfwError);
    } catch (...) {
        std::cout << "Error in trying to call deafultOpenGLSettings\n";
    }
}

JNIEXPORT jlong JNICALL Java_AQSLTranspilerJNI_GLWrap_createWindow
...
JNIEXPORT jboolean JNICALL Java_AQSLTranspilerJNI_GLWrap_glfwWindowShouldClose
(JNIEnv*, jobject, jlong currentWindow) {
    return glfwWindowShouldClose((GLFWwindow*)currentWindow);
}

JNIEXPORT void JNICALL Java_AQSLTranspilerJNI_GLWrap_glfwPollEvents
(JNIEnv*, jobject) {
    glfwPollEvents();
}

JNIEXPORT void JNICALL Java_AQSLTranspilerJNI_GLWrap_glClearColor
(JNIEnv*, jobject, jfloat r, jfloat g, jfloat b, jfloat a) {
    glClearColor(r, g, b, a);
}

JNIEXPORT void JNICALL Java_AQSLTranspilerJNI_GLWrap_glClearColorDepthBuffer
(JNIEnv*, jobject) {
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

JNIEXPORT void JNICALL Java_AQSLTranspilerJNI_GLWrap_glfwSwapBuffers
(JNIEnv*, jobject, jlong currentWindow) {
    glfwSwapBuffers((GLFWwindow*)currentWindow);
}

JNIEXPORT void JNICALL Java_AQSLTranspilerJNI_GLWrap_glfwTerminate
(JNIEnv*, jobject) {
    glfwTerminate();
}

JNIEXPORT void JNICALL Java_AQSLTranspilerJNI_GLWrap_deleteGL
(JNIEnv*, jobject) {
    delete aqOpenGL;
}
